const Store = {
    ...
    getUserData(key) {
        ...
        if (!this.db[userId]) this.db[userId] = { ingredients: [], recipes: [] };
        return this.db[userId][key] || [];
    },
    setUserData(key, val) {
        if (!this.db.currentUser) return;
        const userId = this.db.currentUser.id;
        if (!this.db[userId]) this.db[userId] = { ingredients: [], recipes: [] };
        this.db[userId][key] = val;
        this.save();
    },
    getSettings() {
        ...
        if (!this.db[userId].settings) {
            this.db[userId].settings = { salary: 3000, hours: 160, fixedCosts: [] };
        }
...
function updateDashboard() {
    ...
    if(recipeStat) recipeStat.innerText = recipes.length;
    if(ingredientStat) ingredientStat.innerText = ingredients.length;
    ...
    if(fcList) {
        fcList.innerHTML = '';
        settings.fixedCosts.slice(0, 3).forEach(fc => {
            fcList.innerHTML += `
